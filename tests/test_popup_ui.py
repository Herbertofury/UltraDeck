...
def main():
 ...
 try:
  ...
  c.ev("""globalThis.__messages=[];globalThis.__optionsOpened=0;globalThis.__siteWrites=[];globalThis.__sites={tumblr:true,patreon:true,x:true,tiktok:true};globalThis.__settings={columns:'auto',layoutMode:'masonry',mediaOnly:false,turboMedia:true,minCardWidth:320,minCardHeight:0,gap:16,leftOpen:true,rightOpen:true};globalThis.chrome={runtime:{openOptionsPage:async()=>{__optionsOpened++}},storage:{local:{get:async(k)=>({[k]:k==='ultradeckSites'?{...__sites}:undefined}),set:async(v)=>{if(v.ultradeckSites){__sites={...v.ultradeckSites};__siteWrites.push({...__sites})}}}},tabs:{query:async()=>[{id:42,url:'https://www.tumblr.com/dashboard'}],sendMessage:async(id,msg)=>{__messages.push(JSON.parse(JSON.stringify(msg)));if(msg.type==='setColumns')__settings.columns=msg.value;if(msg.type==='setSettings')Object.assign(__settings,msg.value||{});return {ok:true,payload:{version:'8.5.0',site:'tumblr',siteLabel:'Tumblr',settings:{...__settings},diagnostics:{cachedPosts:128,renderedColumns:Number(__settings.columns)||12,mediaQualityReady:118,mediaPreloadCompleted:118,interactionFailures:0,geometryViolations:0,layoutMode:__settings.layoutMode,minCardHeight:__settings.minCardHeight,mediaOnly:__settings.mediaOnly}}}}}};true""")
  ...
  c.ev("document.getElementById('nav').click();document.getElementById('focus').click();document.getElementById('extras').click();document.getElementById('sync').click();document.getElementById('rebalance').click();document.getElementById('rescan').click();document.getElementById('openOptions').click();document.getElementById('site-tiktok').click();true");time.sleep(.25)
  out=c.ev("({status:document.getElementById('status').textContent,version:document.getElementById('version').textContent,posts:document.getElementById('posts').textContent,media:document.getElementById('media').textContent,minHeightOut:document.getElementById('minHeightOut').textContent,messages:__messages,types:__messages.map(x=>x.type),optionsOpened:__optionsOpened,sites:__sites,siteWrites:__siteWrites})")
  print(json.dumps(out,indent=2))
  assert out['status']=='Live · Tumblr' and out['version']=='v8.5.0' and out['posts']=='128' and out['media']=='118' and out['minHeightOut']=='360px'
  assert out['optionsOpened']==1 and out['sites']['tiktok'] is False and len(out['siteWrites'])>=1
  ...
  c.shot(str(PROJECT/'dist/UltraDeck-Extension-v8.5.0-popup.png'))
 finally:
