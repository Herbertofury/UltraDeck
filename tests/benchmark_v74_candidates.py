...
CAND=pathlib.Path(sys.argv[2])
REVERSE='--reverse' in sys.argv[3:]
RUNS=int(os.environ.get('ULTRADECK_RUNS','5'))
...
CERT=pathlib.Path(tempfile.mkdtemp(prefix='ud-cand-cert-'))
...
def run_version(label,ext,port,browser):
 profile=tempfile.mkdtemp(prefix=f'ud-cand-{label}-');rows=[]
 with sync_playwright() as pw:
  ...
  try:
   ...
   for run in range(1,RUNS+1):
    ...
    t=time.perf_counter();r=p.goto(f'https://www.tumblr.com:{port}/dashboard/following?{label}={run}',wait_until='domcontentloaded',timeout=60000);assert r and r.status==200
    p.wait_for_function("window.__TumblrUltraWideDeck?.version==='7.4.0'",timeout=20000);runtime=(time.perf_counter()-t)*1000
    ...
    row={'run':run,'runtimeMs':round(runtime,2),'ready200Ms':round(ready,2),'longTaskMs':d['longTaskMs'],'fullScans':d.get('fullScans'),'fullScanSkips':d.get('fullScanSkips',0),'retained':p.locator('#tu-ultrawide-deck-grid .tu-item').count(),'geometryViolations':d['geometryViolations'],'interactionFailures':d['interactionFailures'],'errors':errors}
 ...
 return {'label':label,'runs':rows,**{k:{'mean':round(statistics.mean(vals(k)),2),'median':round(statistics.median(vals(k)),2)} for k in ['runtimeMs','ready200Ms','longTaskMs']}}
...
def main():
 ...
 order=[('candidate',CAND),('base',BASE)] if REVERSE else [('base',BASE),('candidate',CAND)]
 try: results=[run_version(label,ext,port,browser) for label,ext in order]
 finally:srv.shutdown();srv.server_close()
 by={r['label']:r for r in results};b=by['base'];n=by['candidate']
 imp={}
 for k in ['runtimeMs','ready200Ms','longTaskMs']:
  for stat in ['mean','median']:
   a=b[k][stat];z=n[k][stat];imp[f'{k}_{stat}']=round((a-z)/a*100,2) if a else None
 out={'browser':browser_diagnostics(),'runsPerBuild':RUNS,'reverse':REVERSE,'base':str(BASE),'candidate':str(CAND),'results':results,'improvementPositiveMeansCandidateFasterPct':imp}
