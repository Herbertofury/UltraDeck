...
BASE=pathlib.Path(sys.argv[1]); CAND=pathlib.Path(sys.argv[2]); REVERSE='--reverse' in sys.argv[3:]; RUNS=5
...
CERT=pathlib.Path(tempfile.mkdtemp(prefix='ud-mut-cert-'))
...
def run_version(label,ext,port,browser):
 profile=tempfile.mkdtemp(prefix=f'ud-mut-{label}-'); rows=[]
 with sync_playwright() as pw:
  ...
  try:
   for run in range(1,RUNS+1):
    ...
    p.goto(f'https://www.tumblr.com:{port}/dashboard/following?{label}={run}',wait_until='domcontentloaded',timeout=60000);p.wait_for_function("window.__TumblrUltraWideDeck?.version==='7.4.0'",timeout=20000);p.wait_for_function("document.querySelectorAll('#tu-ultrawide-deck-grid .tu-item').length===200",timeout=30000);p.wait_for_timeout(120)
 ...
 return {'label':label,'runs':rows,**{k:{'mean':round(statistics.mean(vals(k)),2),'median':round(statistics.median(vals(k)),2)} for k in ['runtimeCpuMs','longTaskDeltaMs']}}
