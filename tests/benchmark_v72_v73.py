...
CERTDIR = pathlib.Path(tempfile.mkdtemp(prefix='ud-72-73-cert-'))
subprocess.run([
...
def main():
    ...
    v72 = [r for r in rows if r['version'] == '7.2.0']
    v73 = [r for r in rows if r['version'] == '7.3.0']
    s72, s73 = summarize(v72), summarize(v73)
    improvements = {}
    for metric in ['runtimeMs','ready200Ms','initialLongTaskMs','syncTransitionMeanMs','syncTransitionMaxMs','bridgeCommandMeanMs','bridgeCommandMaxMs']:
        improvements[metric+'MeanPct'] = pct(s72[metric]['mean'], s73[metric]['mean'])
        improvements[metric+'MedianPct'] = pct(s72[metric]['median'], s73[metric]['median'])
    output = {
    ...
    target = ROOT/'dist/UltraDeck-v7.3.0-performance-ab.json'
