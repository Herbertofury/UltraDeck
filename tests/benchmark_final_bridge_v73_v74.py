...
V72=pathlib.Path('/mnt/data/UltraDeck-Project/source/UltraDeck-Tumblr-Extension-v7.3.0/dist-manual/chromium-mv3');V73=ROOT/'dist-manual/chromium-mv3'
...
def main():
 ...
 try:r=[run('v7.3.0',V72,'7.3.0',port,browser),run('v7.4.0',V73,'7.4.0',port,browser)]
 finally:srv.shutdown();srv.server_close()
 b,n=r;out={'schema':'ultradeck.bridge-hotpath-ab/1','browser':browser_diagnostics(),'cycles':CYCLES,'sequence':SEQ,'results':r,'improvementPositiveMeansFasterPct':{k:round((b[k]-n[k])/b[k]*100,2) for k in ['meanMs','medianMs','p90Ms','maxMs']}};target=ROOT/'dist/UltraDeck-v7.4.0-bridge-hotpath-ab.json';target.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2))
