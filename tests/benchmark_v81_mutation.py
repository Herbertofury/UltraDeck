...
BASE=pathlib.Path(sys.argv[1]); CAND=pathlib.Path(sys.argv[2]); REVERSE='--reverse' in sys.argv[3:]; RUNS=int(os.environ.get('ULTRADECK_RUNS','5')); SETTLE_MS=int(os.environ.get('ULTRADECK_SETTLE_MS','120'))
 ...
 with sync_playwright() as pw:
  ...
  try:
   for run in range(1,RUNS+1):
    ...
    p.goto(f'https://www.tumblr.com:{port}/dashboard/following?{label}={run}',wait_until='domcontentloaded',timeout=60000);p.wait_for_function(f"window.__TumblrUltraWideDeck?.version==='{version}'",timeout=20000);p.wait_for_function("document.querySelectorAll('#tu-ultrawide-deck-grid .tu-item').length===200",timeout=30000);p.wait_for_timeout(SETTLE_MS)
    ...
    row={'run':run,'runtimeCpuMs':round(runtime_cpu_ms(prof),2),'longTaskDeltaMs':round(after['longTaskMs']-before['longTaskMs'],2),'beforeAudits':before.get('geometryAudits',0),'afterAudits':after.get('geometryAudits',0),'auditYieldsDelta':after.get('geometryAuditYields',0)-before.get('geometryAuditYields',0),'retained':p.locator('#tu-ultrawide-deck-grid .tu-item').count(),'geometryViolations':after['geometryViolations'],'interactionFailures':after['interactionFailures'],'errors':errors}
