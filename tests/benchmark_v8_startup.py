...
def manifest_version(p): return json.loads((p/'manifest.json').read_text())['version']
BASE_VERSION=manifest_version(BASE); CAND_VERSION=manifest_version(CAND)
...
CERT=pathlib.Path(tempfile.mkdtemp(prefix='ud-v8-start-cert-'))
...
def run_version(label,ext,version,port,browser):
 profile=tempfile.mkdtemp(prefix=f'ud-v8-start-{label}-');rows=[]
 ...
 return {'label':label,'version':version,'runs':rows,**{k:{'mean':round(statistics.mean(vals(k)),2),'median':round(statistics.median(vals(k)),2)} for k in ['runtimeMs','ready200Ms','longTaskMs']}}
