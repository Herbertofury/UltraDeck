...
def version(path): return json.loads((path/'manifest.json').read_text())['version']
...
CERT=pathlib.Path(tempfile.mkdtemp(prefix='ud-v8-hot-cert-'));subprocess.run(['openssl','req','-x509','-newkey','rsa:2048','-nodes','-keyout',str(CERT/'key.pem'),'-out',str(CERT/'cert.pem'),'-days','1','-subj','/CN=www.tumblr.com'],check=True,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
...
def run(label,ext,version,port,browser):
 prof=tempfile.mkdtemp(prefix=f'ud-v8-hot-{label}-'); errs=[]
